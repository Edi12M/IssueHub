...
import { MOCK_ISSUES, STATUS_SLUG, STATUS_TO_SLUG, isOverdue } from "../../data/mockIssues";
  ...
  return (
    <DevShell>
      ...
      <div style={{ fontSize:13, color:C.muted, marginBottom:24 }}>
        {shown.length} issue{shown.length !== 1 ? "s" : ""} · filtered by{" "}
      ...
      <div style={{ display:"flex", gap:10, marginBottom:20, flexWrap:"wrap" }}>
        {FILTERS.map((f) => {
          ...
          return <FilterButton key={f} label={f} active={f === activeFilter} to={to} />;
        })}
      </div>
