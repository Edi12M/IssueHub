...
import {
  DevShell, BackLink, PriorityTag, StatusTag,
  Avatar, SectionLabel, Card, TinyBtn, C,
...
function IssueDetail({ source }) {
  ...
  function handleStatusUpdate() {
    ...
    setIssue((prev) => ({
      ...prev,
      status: newStatus,
      statusHistory: [...prev.statusHistory, { status:newStatus, date:now, by:"Alex Rivera" }],
    }));
  }
  ...
  function handleAddComment() {
    ...
    setIssue((prev) => ({
      ...
      comments: [...prev.comments, {
        id:"c" + Date.now(), author:"Alex Rivera", avatar:"AR",
        text:commentText.trim(), date:now, mine:true,
      }],
    }));
    setCommentText("");
  }
  ...
  function handleFileUpload(e) {
    ...
    setAttachments((prev) => [
      ...
      ...files.map((f) => ({
        ...
        type: f.name.match(/\.(png|jpg|jpeg|gif|webp)$/i) ? "img"
            : f.name.match(/\.txt$/i) ? "txt" : "fig",
        date: today(),
      })),
    ]);
  }
  ...
  function AttachIcon({ type }) {
    ...
    return (
      <div style={{ width:32, height:32, borderRadius:6, background:m.bg,
  ...
  return (
    <DevShell>
      ...
      <div style={{ fontSize:22, fontWeight:700, color:C.textHi, marginBottom:20 }}>
        {issue.title}
      </div>
        ...
        <div>
          ...
          <Card>
            ...
            {issue.comments.length === 0 && (
              <div style={{ color:C.subtle, fontSize:13, marginBottom:12 }}>No comments yet.</div>
            )}
              ...
              <div key={c.id} style={{
                ...
                <div style={{ display:"flex", alignItems:"center",
                  ...
                  <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                    ...
                    <span style={{ fontSize:12, fontWeight:700, color:"#a78bfa" }}>{c.author}</span>
                  </div>
                ...
                {editingComment?.id === c.id ? (
                  <>
                    <textarea
                      ...
                      style={{ width:"100%", background:"#0f1117", border:`1px solid ${C.border}`,
                        borderRadius:8, color:C.text, padding:"10px 12px", fontSize:13,
                        resize:"vertical", minHeight:72, marginBottom:8,
                  ...
                  <>
                    <div style={{ fontSize:13.5, color:"#cbd5e1", lineHeight:1.6 }}>{c.text}</div>
                    {c.mine && (
                      <div style={{ display:"flex", gap:8, marginTop:8 }}>
                        <TinyBtn onClick={() => setEditingComment({ id:c.id, text:c.text })}>Edit</TinyBtn>
                        <TinyBtn danger onClick={() => handleDeleteComment(c.id)}>Delete</TinyBtn>
                      </div>
                    )}
                  </>
                )}
          ...
          <Card>
            ...
            <div onClick={() => fileRef.current.click()}
              style={{ border:`2px dashed ${C.border}`, borderRadius:10,
                ...
                fontSize:13, color:C.subtle, marginTop:12 }}>
        ...
        <div>
          ...
          <Card>
            ...
            <select
              ...
              style={{ background:"#0f1117", border:`1px solid ${C.border}`,
                borderRadius:8, color:C.text, padding:"8px 12px",
                fontSize:13, width:"100%", cursor:"pointer", marginBottom:10 }}>
            ...
            {issue.statusHistory.map((h, i) => (
              <div key={i} style={{ display:"flex", gap:12,
                ...
                <div style={{ width:8, height:8, borderRadius:"50%",
                  background:C.accent, marginTop:5, flexShrink:0 }} />
                <div>
